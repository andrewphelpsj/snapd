...
func (m *DeviceManager) doInstallFinish(t *state.Task, _ *tomb.Tomb) error {
	...
	if !sys.Model.Classic() {
		...
		if copier, ok := sysSeed.(seed.Copier); ok {
			...
			err = sysSeed.LoadAssertions(db, func(b *asserts.Batch) error {
			...
			err = sysSeed.LoadMeta(seed.AllModes, nil, timings.New(nil))
			if err != nil {
				return fmt.Errorf("cannot load seed metadata: %w", err)
			}

			logger.Debugf("copying label %q to seed partition", sys.Label)
			if err := copier.Copy(seedMntDir); err != nil {
