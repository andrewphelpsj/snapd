...
func (s *deviceMgrInstallAPISuite) mockSystemSeedWithLabel(c *C, label string, isClassic, hasPartial bool, seedCopyFn func(string) error) (gadgetSnapPath, kernelSnapPath string, ginfo *gadget.Info, mountCmd *testutil.MockCmd) {
...
type fakeSeedCopier struct {
	fakeSeed
	copyFn func(root string) error
}

func (s *fakeSeedCopier) Copy(root string) error {
	return s.copyFn(root)
}
...
func (s *deviceMgrInstallAPISuite) testInstallFinishStep(c *C, opts finishStepOpts) {
	...
	seedCopyFn := func(root string) error { return fmt.Errorf("unexpected copy call") }
	seedCopyCalled := false
	if !opts.installClassic {
		seedCopyFn = func(root string) error {
			c.Check(root, Equals, filepath.Join(dirs.RunDir, "mnt/ubuntu-seed"))
			seedCopyCalled = true
			return nil
		}
	}
