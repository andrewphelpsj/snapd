...
func (s *rebootSuite) componentExclusiveInstallTaskSetForSnapSetup(snapName string, snapType snap.Type) snapstate.SnapInstallTaskSet {
	...
	downloadComp.WaitFor(setupSecurity)
	...
	linkComp.WaitFor(downloadComp)

	ts := state.NewTaskSet(setupSecurity, downloadComp, linkComp)
	ts.MarkEdge(setupSecurity, snapstate.BeginEdge)
	ts.MarkEdge(setupSecurity, snapstate.SnapSetupEdge)
	ts.MarkEdge(linkComp, snapstate.EndEdge)
...
func (s *rebootSuite) TestArrangeSnapInstallTaskSetsSeedRefreshComponentExclusive(c *C) {
	...
	var downloadComp *state.Task
	for _, haltTask := range snapSetupTask.HaltTasks() {
		if haltTask.Kind() == "download-component" {
			downloadComp = haltTask
			break
		}
	}

	c.Check(seedCreate.WaitTasks(), testutil.Contains, snapSetupTask)
	c.Check(snapSetupTask.WaitTasks(), Not(testutil.Contains), seedFinalize)
	c.Assert(downloadComp, NotNil)
	c.Check(downloadComp.WaitTasks(), testutil.Contains, seedFinalize)
}
