...
func (s *snapmgrTestSuite) testUpdateWithComponentsRunThrough(c *C, opts updateWithComponentsOpts) {
	...
	if withKMods {
		...
		if len(opts.postRefreshComponents) > 0 {
			...
			// verify MaybeRebootWaitEdge points to the first link-component
			// task
			if linkTasks := tasksWithKind(ts, "link-component"); len(linkTasks) > 1 {
				c.Check(postRebootTask.ID(), Equals, linkTasks[0].ID(),
					Commentf("MaybeRebootWaitEdge should point to first link-component task"))
			}
		} else {
