...
func (s *snapmgrTestSuite) TestUpdateWithComponentsBackToPrevRevision(c *C) {
	...
	// 1. current component revisions
	// 2. previous component revisions
	// 3. new component revisions (that is connected with the original snap
	//    revision, via the store)
...
func (s *snapmgrTestSuite) testUpdateWithComponentsRunThrough(c *C, opts updateWithComponentsOpts) {
	...
	if opts.undo {
		expected = append(expected, undoOps(instanceName, newSnapRev, currentSnapRev, opts.postRefreshComponents, updatedCompRevisions, originalCompRevisions)...)
	} else {
		expected = append(expected, fakeOp{
			op:    "cleanup-trash",
			name:  instanceName,
...
func (s *snapmgrTestSuite) testUpdateWithComponentsRunThroughOnlyComponentUpdate(c *C, opts updateWithComponentsOpts) {
	...
	for _, compName := range opts.postRefreshComponents {
		containerName := fmt.Sprintf("%s+%s", instanceName, compName)
		removedFilename := fmt.Sprintf("%s_%v.comp", containerName, originalCompRevisions[compName])
		expected = append(expected, []fakeOp{
			{
				op:   "link-component",
				path: snap.ComponentMountDir(compName, updatedCompRevisions[compName], instanceName),
	...
	if opts.undo {
		expected = append(expected, undoOps(
			instanceName, currentSnapRev, currentSnapRev, opts.postRefreshComponents, updatedCompRevisions, originalCompRevisions)...,
		)
	} else {
		expected = append(expected, fakeOp{
			op:    "cleanup-trash",
	...
	if !opts.undo {
		...
		c.Assert(snapst.Sequence.Revisions[0], DeepEquals, cand)
	} else {
		// make sure everything is back to how it started
		c.Assert(snapst.Active, Equals, true)
		c.Assert(snapst.Sequence.Revisions, HasLen, 1)
		c.Assert(snapst.Sequence.Revisions[0].Snap, DeepEquals, seq.Revisions[0].Snap)

		// TODO: figure out why this is out of order and if it is a problem
		c.Assert(snapst.Sequence.Revisions[0].Components, testutil.DeepUnsortedMatches, seq.Revisions[0].Components)
	}
}
