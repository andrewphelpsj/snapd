...
func (s *snapmgrTestSuite) TestInstallFailsOnDisabledSnap(c *C) {
	...
	_, _, err := snapstate.DoInstallOrPreDownload(s.state, snapst, snapsup, nil, snapstate.InstallContext{})
...
func (s *snapmgrTestSuite) TestInstallFailsOnSystem(c *C) {
	...
	_, _, err := snapstate.DoInstallOrPreDownload(s.state, nil, snapsup, nil, snapstate.InstallContext{})
...
func (s *snapmgrTestSuite) TestInstallNoRestartBoundaries(c *C) {
	...
	_, ts1, err := snapstate.DoInstallOrPreDownload(s.state, &snapstate.SnapState{}, snapsup, nil, snapstate.InstallContext{})
	c.Assert(err, IsNil)
	...
	_, ts2, err := snapstate.DoInstallOrPreDownload(s.state, &snapstate.SnapState{}, snapsup, nil, snapstate.InstallContext{NoRestartBoundaries: true})
	c.Assert(err, IsNil)
