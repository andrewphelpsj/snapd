...
func (s *snapmgrTestSuite) TestDownloadSpecifyDownloadDir(c *C) {
	...
	ts, info, err := snapstate.Download(context.Background(), s.state, "foo", downloadDir, nil, snapstate.RevisionOptions{
		Revision: snap.R(1),
	}, snapstate.Options{})
	c.Assert(err, IsNil)
...
func (s *snapmgrTestSuite) TestDownloadAlreadyInstalled(c *C) {
	...
	const downloadDir = ""
	_, _, err := snapstate.Download(context.Background(), s.state, "foo", downloadDir, nil, snapstate.RevisionOptions{}, snapstate.Options{})
	c.Assert(err, NotNil)
