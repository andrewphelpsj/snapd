...
func storeUpdateSummary(
	...
	for snapst, revOpts := range hasLocalRevision {
		...
		// TODO: for now, go back to the components that were already installed
		// with this revision. to be more robust, we'd need to compare what
		// components are installed with the revision that we are refreshing
		// from to the components available in the store for the revision that
		// we are refreshing to.
