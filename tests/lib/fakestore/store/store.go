...
func (s *Store) detailsEndpoint(w http.ResponseWriter, req *http.Request) {
	...
	details := detailsReplyJSON{
		...
		AnonDownloadURL: fmt.Sprintf("%s/download/%s", s.URL(), filepath.Base(sn.path)),
		DownloadURL:     fmt.Sprintf("%s/download/%s", s.URL(), filepath.Base(sn.path)),
...
func (s *Store) snapActionEndpoint(w http.ResponseWriter, req *http.Request) {
	...
	for _, a := range actions {
		...
		res.Snap.Download.URL = fmt.Sprintf("%s/download/%s", s.URL(), filepath.Base(sn.path))
