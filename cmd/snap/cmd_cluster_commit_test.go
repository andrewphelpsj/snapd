...
func (s *clusterCommitSuite) TestClusterCommitClientInteraction(c *check.C) {
	...
	s.RedirectClientToTestServer(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/v2/cluster/uncommitted":
			if r.Method == "GET" {
				...
				postCalled = true
				// verify we got an assertion
				var req map[string]string
				c.Assert(json.NewDecoder(r.Body).Decode(&req), check.IsNil)
				
				assertionStr, ok := req["assertion"]
				c.Assert(ok, check.Equals, true)
				
				// just verify it's a valid assertion
	...
	// test CommitClusterAssertion
	err = client.CommitClusterAssertion(clusterAssert.(*asserts.Cluster))
	c.Assert(err, check.IsNil)
