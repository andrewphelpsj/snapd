...
func createExpiredSerial(c *check.C, signing *assertstest.StoreStack) string {
	// create a device key for this assertion
	deviceKey, _ := assertstest.GenerateKey(1024)
	encDevKey, err := asserts.EncodePublicKey(deviceKey.PublicKey())
	if err != nil {
		c.Fatal(err)
	}

	// create assertion with timestamp from 2 years ago
	serial, err := signing.Sign(asserts.SerialType, map[string]any{
		"brand-id":            "brand",
		"model":               "model",
		"serial":              randutil.RandomString(15),
	...
	return string(asserts.Encode(serial))
}
...
func (s *deviceTrackerSuite) TestDatabaseIntegrationTrustedKeys(c *check.C) {
	data := assemblestate.DeviceQueryTrackerData{}
	db, signing := mockAssertDB(c)
	dt, err := assemblestate.NewDeviceQueryTracker(data, time.Minute, time.Now, db, "test-secret")
	c.Assert(err, check.IsNil)

	// create assertion signed by trusted key
	serial, key := createTestSerial(c, signing)
...
func (s *deviceTrackerSuite) TestDatabaseIntegrationUntrustedKeys(c *check.C) {
	// create database with limited trusted keys
	trustedAccounts := assertstest.NewStoreStack("canonical", nil)
	db, err := asserts.OpenDatabase(&asserts.DatabaseConfig{
		Backstore: asserts.NewMemoryBackstore(),
		Trusted:   trustedAccounts.Trusted,
	})
	c.Assert(err, check.IsNil)
	...
	// create assertion signed by different (untrusted) authority
	untrusted := assertstest.NewStoreStack("untrusted-authority", nil)
	serial, _ := createTestSerial(c, untrusted)

	// test that identity with untrusted assertion is still recorded (graceful failure)
	identity := assemblestate.Identity{
		RDT:    assemblestate.DeviceToken("test-device"),
		Serial: string(asserts.Encode(serial)),
	}
...
func (s *deviceTrackerSuite) TestDatabaseIntegrationChainOfTrust(c *check.C) {
	data := assemblestate.DeviceQueryTrackerData{}
	db, signing := mockAssertDB(c)
	dt, err := assemblestate.NewDeviceQueryTracker(data, time.Minute, time.Now, db, "test-secret")
	c.Assert(err, check.IsNil)

	// create assertion with proper chain of trust and use the same key for signing
	serial, deviceKey := createTestSerial(c, signing)

	// test that identity with proper chain of trust is recorded successfully
	testRDT := assemblestate.DeviceToken("test-device")
	testFP := assemblestate.Fingerprint{1, 2, 3, 4, 5}

	// create valid SerialProof using the SAME device key that was used in the serial assertion
	expectedHMAC := assemblestate.CalculateHMAC(testRDT, testFP, "test-secret")
	validProof, err := asserts.SignWithKey(expectedHMAC, deviceKey)
	c.Assert(err, check.IsNil)
	...
	identity := assemblestate.Identity{
		...
		FP:          testFP,
		Serial:      string(asserts.Encode(serial)),
		SerialProof: assemblestate.Proof(validProof),
	}
	err = dt.RecordIdentity(identity)
	c.Assert(err, check.IsNil)
	c.Assert(dt.Identified("test-device"), check.Equals, true)
...
func (s *deviceTrackerSuite) TestSerialProofValidation(c *check.C) {
	...
	cases := []struct {
		...
		},
		{
			...
			err: ".*serial proof verification failed.*cannot decode signature.*",
		},
	}
...
func (s *deviceTrackerSuite) TestExpiredAssertion(c *check.C) {
	...
	serial := createExpiredSerial(c, signing)
	identity := assemblestate.Identity{
		RDT:    assemblestate.DeviceToken("test-device"),
		Serial: serial,
	}
