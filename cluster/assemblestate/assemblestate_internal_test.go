...
// createTestAssembleConfig creates a complete AssembleConfig for testing
func createTestAssembleConfig(c *check.C, signing *assertstest.StoreStack, ip net.IP, secret, rdt string, port int) AssembleConfig {
...
func (s *clusterSuite) TestPublishDevicesAndCommit(c *check.C) {
	...
	client.TrustedFunc = func(ctx context.Context, addr string, cert []byte, kind string, message any) error {
		...
		// verify the serial contains the full assertion, not just the serial number
		c.Assert(len(d.Serial) > 0, check.Equals, true)
		c.Assert(len(d.SerialProof) > 0, check.Equals, true)
		...
		err := asserts.VerifyWithKey(expectedHMAC, d.SerialProof, key.PublicKey())
		c.Assert(err, check.IsNil)
...
func (s *clusterSuite) TestRecordDevicesForgedIdentity(c *check.C) {
	...
	cases := []struct {
	...
	}{
		{
			...
			identity: func() Identity {
				...
				proof, err := asserts.SignWithKey(hmac, attackerKey)
				c.Assert(err, check.IsNil)
		...
		},
		{
			...
			identity: func() Identity {
				...
				// create valid proof
				proof, err := asserts.SignWithKey(hmac, deviceKey)
				c.Assert(err, check.IsNil)
		...
		},
		{
			...
			identity: func() Identity {
				...
				proof, err := asserts.SignWithKey(wrongHMAC, deviceKey)
				c.Assert(err, check.IsNil)
	...
	for _, tc := range cases {
		c.Logf("testing forged identity case: %s", tc.name)
...
func (s *clusterSuite) TestNewAssembleStateTimeout(c *check.C) {
	ip := net.IPv4(127, 0, 0, 1)
	db, signing := mockAssertDB(c)
	cfg := createTestAssembleConfig(c, signing, ip, "secret", "self", 8001)
	now := time.Now()
...
func (s *clusterSuite) TestNewAssembleStateWithSessionImport(c *check.C) {
	...
	serial, deviceKey := createTestSerial(c, signing)
	proof, err := asserts.SignWithKey(CalculateHMAC(local.rdt, local.fp, "secret"), deviceKey)
	c.Assert(err, check.IsNil)
...
func (s *clusterSuite) TestNewAssembleStateInvalidSessionData(c *check.C) {
	ip := net.IPv4(127, 0, 0, 1)
	db, signing := mockAssertDB(c)
	cfg := createTestAssembleConfig(c, signing, ip, "secret", "rdt", 8001)
		...
		},
		{
			...
			session: func() AssembleSession {
				...
				return AssembleSession{
					Devices: DeviceQueryTrackerData{
						IDs: []Identity{
							createTestIdentity(c, signing, cfg.RDT, wrongFP, "secret"),
						},
					},
				}
			}(),
...
func (s *clusterSuite) TestMaxSizeCompletion(c *check.C) {
	ip := net.IPv4(127, 0, 0, 1)
	db, signing := mockAssertDB(c)
	cfg := createTestAssembleConfig(c, signing, ip, "secret", "one", 8001)
