...
for host in ${hosts}; do
    ...
    echo snap cluster assemble --secret="${secret}" --address="${addr}:8080" --domain=multipass --no-wait
...
addr=$(multipass exec "${host}" bash -- -c "ip -4 -o addr show dev ens3 | awk '{print \$4}' | cut -d/ -f1")
echo snap cluster assemble --secret="${secret}" --address="${addr}:8080" --domain=multipass --expected-size="${count}"
multipass exec "${host}" sudo -- snap cluster assemble --secret="${secret}" --address="${addr}:8080" --domain=multipass --expected-size="${count}"
