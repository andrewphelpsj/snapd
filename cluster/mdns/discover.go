...
// addresses observed for peers of the same service type.
func MulticastDiscovery(ctx context.Context, cfg Config) (<-chan string, func(), error) {
	if cfg.Interface == "" {
	...
	if cfg.IP != nil {
		svcConfig.IPs = []net.IP{cfg.IP}
	}

	sv, err := dnssd.NewService(svcConfig)
	if err != nil {
		return nil, nil, err
	}
