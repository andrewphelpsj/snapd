...
func (s *snapassertsSuite) TestCrossCheckResourceErrors(c *C) {
	...
	tests := []test{
		...
		},
		{
			...
			err: `resource \"test-component\" does not have expected snap ID or revision according to assertions \(metadata is broken or tampered\): 24 / snap-id-1 != 25 / snap-id-1`,
		},
		{
			pairOverrides: map[string]interface{}{
				"resource-revision": "25",
