...
func findResourcePair(name, snapID string, resourceRev, snapRev int, provenence string, db Finder) (*asserts.SnapResourcePair, error) {
...
func CrossCheckResource(name, hash, provenance string, size uint64, csi *snap.ComponentSideInfo, si *snap.SideInfo, model *asserts.Model, db Finder) error {
	...
	if resrev.SnapID() != si.SnapID || resrev.ResourceRevision() != csi.Revision.N {
		return fmt.Errorf(
			"resource %q does not have expected snap ID or revision according to assertions (metadata is broken or tampered): %s / %s != %d / %s",
			name, csi.Revision, si.SnapID, resrev.ResourceRevision(), resrev.SnapID(),
		)
	}
...
// snap-resource-revision's provenence with the snap-declaration's revision
// authority.
